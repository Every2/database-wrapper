...
static bool cmd_is(const std::string& word, const std::string& cmd) {
...
static bool try_one_request(Conn *conn) {
    ...
    if (remain) {
        memmove(&conn->rbuf, &conn->rbuf.at(4 + len), remain);
    }
...
static bool try_fill_buffer(Conn *conn) {
    ...
    do {
        ...
        rv = read(conn->fd, &conn->rbuf.at(conn->rbuf_size), cap);
...
static bool try_flush_buffer(Conn *conn) {
    ...
    do {
        ...
        rv = write(conn->fd, &conn->wbuf.at(conn->wbuf_sent), remain);
    } while (rv < 0 && errno == EINTR);
    if (rv < 0 && errno == EAGAIN) {
        return false;
    }
...
int main() {
    ...
    while (true) {
        ...
        for (size_t i {1}; i < poll_args.size(); ++i) {
            if (poll_args.at(i).revents) {
                Conn *conn = fd2conn.at(poll_args.at(i).fd);
                connection_io(conn);
                if (conn->state == STATE_END) {
                    fd2conn.at(conn->fd) = NULL;
